...
var
  Form1: TForm1;
  flag: Boolean;
  m, k, kg, l, dx, dt, tgl, t, up, uk, u: Real;
  x, v, F: array [1..100] of Real;
...
begin
  ...
  while (t < tgl) and flag do
    begin
      ...
      uk := m * sqr(v[1]) / 2 + m * sqr(v[n]) / 2;
      up := kg * sqr(l - x[1]) / 2 + k * sqr(l - (x[n] - x[n-1])) / 2
            + kg * sqr(l - ((n+1) * l - x[n])) / 2;

      for i := 2 to n-1 do
        begin
          F[i] := -k * (2 * x[i] - x[i+1] - x[i-1]);
          uk := uk + m * sqr(v[i]) / 2;
          up := up + k * sqr(l - (x[i]-x[i-1])) / 2;
        end;
      ...
      series2.AddXY(0, 0);
      series2.AddXY(x[1] * 0.3, 0);
      series2.AddXY(x[1] * 0.4, l * 0.01);
      series2.AddXY(x[1] * 0.5, -l * 0.01);
      series2.AddXY(x[1] * 0.6, l * 0.01);
      series2.AddXY(x[1] * 0.7, 0);
      series2.AddXY(x[1], 0);
      series2.AddXY(x[n], 0);
      series2.AddXY(x[n] + ((n+1) * l - x[n]) * 0.3, 0);
      series2.AddXY(x[n] + ((n+1) * l - x[n]) * 0.4, l * 0.01);
      series2.AddXY(x[n] + ((n+1) * l - x[n]) * 0.5, -l * 0.01);
      series2.AddXY(x[n] + ((n+1) * l - x[n]) * 0.6, l * 0.01);
      series2.AddXY(x[n] + ((n+1) * l - x[n]) * 0.7, 0);
      series2.AddXY((n+1) * l, 0);
      ...
      for i := 1 to n do
        begin
          ...
          begin
            ...
            series2.AddXY(x[i] + (x[i + 1] - x[i]) * 0.4, l * 0.01);
            series2.AddXY(x[i] + (x[i + 1] - x[i]) * 0.5, -l * 0.01);
            series2.AddXY(x[i] + (x[i + 1] - x[i]) * 0.6, l * 0.01);
